...
class BJLL {
	...
	public function enqueue_scripts() {
		...
		$jsver = 2;
		wp_enqueue_script( 'BJLL', plugins_url( 'js/lazysizes.min.js', dirname( __FILE__ ) ), null, $jsver, true );
	...
	public static function filter_images( $content ) {
		...
		foreach ( $matches[0] as $imgHTML ) {
			...
			if ( ! preg_match( "/src=['\"]data:image/is", $imgHTML ) ) {
				...
				$replaceHTML = preg_replace( '/<img(.*?)src=/is', '<img$1src="' . esc_attr( $placeholder_url ) . '" data-lazy-type="image" data-src=', $imgHTML );
	//			$replaceHTML = preg_replace( '/<img(.*?)src=/is', '<img$1src="' . esc_attr( $placeholder_url ) . '" data-lazy-type="image" data-src=', esc_attr( $placeholder_url ) );
